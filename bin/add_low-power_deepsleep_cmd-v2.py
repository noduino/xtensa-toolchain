...
class ESPFirmwareImage:
	...
	if bin_type == "flash.bin" or bin_type == "boot.bin":
	...
	elif "flash" in filename or "boot" in filename:
	    len_list = len(segment_list)
	    #print "len_list",len_list	    
	    for i in range(len_list):
		(addr,data)=segment_list[len_list-1-i]
	...
	if "flash" in filename or "boot" in filename:
	    f = file(filename, 'wb')
...
def add_disable_rf():
    ...
    elif "boot" in bin_file_name:
	if True:#try:
	    add_instruction(bin_file=bin_file_name, bin_type="boot.bin", 
	            segment_list=[(0x60000020,struct.pack('<i',0x0004001C)),
				  (0x60000710,struct.pack('<i',0x00000000)),#disable rf 
				  ])    
	#except:
